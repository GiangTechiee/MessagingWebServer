...
interface ConversationWithParticipants {
  ...
  >;
  oppositeUser?: { userId: string; username: string; avatar: string | null };
}
...
export class ConversationService {
  ...
  private async resizeImageIfNeeded(
    ...
    try {
      ...
      console.error('ConversationService: Lỗi khi resize ảnh với sharp', {
        error: error.message,
        stack: error.stack,
      });
  ...
  private async deleteOldFile(url: string | null): Promise<void> {
    ...
    try {
      ...
      console.error(
        'ConversationService: Lỗi khi xóa file cũ trên Cloudinary',
        {
          publicId,
          error: error.message,
          stack: error.stack,
  ...
  private validateConversationCreationInput(
    ...
    users: { userId: string; username: string; avatar: string | null }[],
  ): void {
  ...
  private createConversationResponse(
    ...
    const response: ConversationResponseDto = {
      ...
      groupAvatar: conversation.groupAvatar || 'default-group-avatar.png',
    ...
    if (
      ...
      if (oppositeParticipant) {
        response.oppositeUser = {
          ...
          avatar: oppositeParticipant.user.avatar || 'default-avatar.png',
        };
      }
    } else if (
      conversation.type === ConversationType.DIRECT &&
      ...
      response.oppositeUser = {
        ...
        avatar: conversation.oppositeUser.avatar || 'default-avatar.png',
      };
    }
    ...
    return this.prisma.executeTransaction(async (prisma) => {
      ...
      if (groupAvatar && type === ConversationType.GROUP) {
        ...
        try {
          ...
          console.error(
            'ConversationService: Lỗi khi upload avatar nhóm lên Cloudinary',
            {
              error: error.message,
              stack: error.stack,
            },
      ...
      const conversation = await prisma.conversations.create({
        data: {
          ...
          groupAvatar:
            type === ConversationType.DIRECT
              ? null
              : uploadedAvatarUrl || 'default-group-avatar.png',
          createdAt: new Date(),
          creatorId: userId,
        },
